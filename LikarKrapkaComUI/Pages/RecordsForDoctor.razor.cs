...
{
    public partial class RecordsForDoctor
    {
        ...
        async void Refresh()
        {
            var res = await RecordsService.GetRecordsByDoctorId(doctorId);
        ...
        {
            EditContext.DataItem.DoctorId = doctorId;
            EditContext.DataItem.PatientId = EditContext.PatientId;
        ...
        private async Task<IEnumerable<PersonList>> GetListOfPatients(CancellationToken ct = default)
        {
        ...
        { 
            ...
            var patientsDate = await RecordsService.GetDateForPatient(EditContext.PatientId);
            var doctorsDate = await RecordsService.GetDateForDoctor(doctorId);
        ...
        async Task OnRowRemoving(Record dataItem)
        {
            ...
            if (!result)
                toastService.ShowError("Неможливо видалити звпис, оскільки в базі є пов'язані з нею дані");
            Refresh();
            await InvokeAsync(StateHasChanged);
            await grid.Refresh();
        }
